...
void RenderFilledTriangle(GameObject_Triangle* triangle, char* Buf, int width, int height) {
    ...
    // 1. 삼각형 테두리를 그리기
    Elf2DDrawLine2((int)p1.x, (int)p1.y, (int)p2.x, (int)p2.y, Buf, width, height);
    Elf2DDrawLine2((int)p2.x, (int)p2.y, (int)p3.x, (int)p3.y, Buf, width, height);
    Elf2DDrawLine2((int)p3.x, (int)p3.y, (int)p1.x, (int)p1.y, Buf, width, height);
...
void RenderDot(GameObject_Dot* dot, char* Buf, int width, int height) {
    int x = ((int)dot->Position.x) & ~1; // 짝수로 강제
